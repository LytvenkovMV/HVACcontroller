...
void PRINT_Decimal1Place(int16_t value, PRINT_Alignment Alignment, char result[6])
{
	...
	if (value > 999)
	{
		if (Alignment == PRINT_RIGHT_ALIGNMENT) strcpy(result, " FF.F");
		else strcpy(result, "FF.F");
		return;
	}

	/* If value less than -999 return overflow code */
	if (value < -999)
	{
		strcpy(result, "-FF.F");
		return;
